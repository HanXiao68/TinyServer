...
之后，实现了多传感器的多线程机制，使用互斥锁等机制实现线程间的同步。
...
> * 使用ROS的编程机制，首先创造rosnodehandle句柄，然后使用这个句柄进行对多个终端的数据订阅处理。在subscriber函数中，需要传入 ：1.被订阅话题的名称（包括LiDAR，IMU，camera等终端作为客户端发布的数据） 2.队列缓冲大小， 3，函数指针，也就是执行数据处理的回调函数。
...
> * 作为webserver，经过webbench的压测，能够实现4680QPS。
> * 能够通过这个服务，运用到后台开发的相关知识，锻炼自己的开发能力。

### 不足和展望
> * 由于我目前的条件有限，只能在单机上做多线程。其实这个问题，在例如很多的大厂肯定有着完备的分布式 服务器来支持无人驾驶。因此，下一个版本计划，将服务器做成分布式，能够实现负载均衡，加入Nginx。
